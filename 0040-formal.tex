...
In case, of static benchmarks many of the parameters may be fixed. However, when defining dynamic benchmarks, we define a metric that is to bi minimized while allowing a predefined set of parameters of the benchmark to be variable. Let $B_i(M)$ denote a benchamrk with a fixed metric M and variations in I, D, T, C, R  specified by i. We try to identify the minimum 

\[ min \{ B_i(..., M, ...)(S_j) \mid \forall_j M(S_j)\} \]

where $M(S_j)$ is the value of the solution for the metric.

Let us assume M denotes the scientific accuracy of the benchmark. In this case we look for the best scientific solution. In many cases however we still have restrictions posed on the benchmark. While it is common to restrict the dataset, the variation in the algorithm used to minimize is often desired. This is based on the fact that in the scientific community we are not only interested in comparing hardware, but to actually fond the best algorithmic solution. Such a solution can then compared further while introducing efficiency or cost metrics comparing the cost it takes to achieve the best solution.
...
However, recent efforts have shown that, in some cases, we need to consider live data ingestion into benchmarks, for example, in earth science or health care applications, to support real-time predictions.
We term such datasets {\em living datasets}, which are continuously updated with new data, edge cases, or corrections.
Such living datasets could be real-time data, but could also include a simulation of real-time data while using a static dataset that is simulating the real time data update. While modifying the dataset the benchmark could evolve over time as the data available may be growing or become more accurate, supporting the need to identify the most accurate solution.
...
A complex example of a difficult task, evaluated by a complex metric, is the problem of protein structure prediction as formalized by CASP.
Their main metric, the Global Distance Test (GDT) encompasses a combination of relevant parameters that a protein structure prediction model should perform well on.
In contrast, benchmarks that use simple metrics, such as the mean square error (MSE) or a static accuracy measurement, have been repeatedly criticised for not reflecting real-world performance.
