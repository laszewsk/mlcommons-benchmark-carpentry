...
In some more complex task the task itself may be a scientific workflow comprised of interacting components. In that case we may use a graph specification of the scientific task that uses subtasks that are augmented in how they relate with each other through edges and indicating data flows and temporal executions. In that case we can also use $W$ instead of $T$ the specification of a workflow with properly augmented edges. Each task could have its own benchmark.

Formally, $W = (T, E)$. Where, 
$T$ represents the collection of all tasks 
...
If the  workflow can not be specified in this fashion a clear specification of the workflow should be presented.

The introduction of Workflows into the formal definition is also motivated by the recent utilization of {\em Agentic AI frameworks} that allows the integration of tasks that may or may not be using AI as benchmark, but its overall performance is important to complete the scientific task set out to complete.
